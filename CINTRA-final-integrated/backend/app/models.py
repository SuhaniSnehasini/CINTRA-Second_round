...
# Encrypted evidence metadata is deliberately separate from EvidenceRecord.
# EvidenceRecord keeps the existing recognition/integrity-chain workflow intact,
# while this table owns the AES-GCM artifact and its custody history.
class Evidence(Base):
    ...
    original_badge_id: Mapped[str | None] = mapped_column(String(64), nullable=True)
    current_custodian: Mapped[str | None] = mapped_column(String(64), nullable=True)
    collector_name: Mapped[str | None] = mapped_column(String(120), nullable=True)
    collector_agency: Mapped[str | None] = mapped_column(String(160), nullable=True)
    collection_timestamp: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    status: Mapped[str] = mapped_column(String(40), default="IN_CUSTODY", nullable=False)
...
class CustodyEvent(Base):
    ...
    actor_badge_id: Mapped[str | None] = mapped_column(String(64), nullable=True)
    from_custodian: Mapped[str | None] = mapped_column(String(64), nullable=True)
    to_custodian: Mapped[str | None] = mapped_column(String(64), nullable=True)
    reason: Mapped[str | None] = mapped_column(Text, nullable=True)
    file_sha256: Mapped[str] = mapped_column(String(64), nullable=False)
    timestamp: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
    blockchain_tx_id: Mapped[str | None] = mapped_column(String(128), nullable=True)
    blockchain_status: Mapped[str] = mapped_column(String(40), default="NOT_RECORDED", nullable=False)
