...
def serialize_person(db: Session, person: Suspect, detailed: bool = False) -> dict:
    ...
    payload = {
        ...
        "suspect_id": person.suspect_code,
...
def _day_counts(db: Session, result: str | None, days: int) -> list[dict]:
    ...
    if result == "MATCH":
        return [{"date": item["date"], "count": item["matches"], "value": item["matches"]} for item in series]
    if result == "NO_MATCH":
        return [{"date": item["date"], "count": item["no_matches"], "value": item["no_matches"]} for item in series]
    return [{"date": item["date"], "count": item["scans"], "value": item["scans"]} for item in series]
...
def analytics_overview(db: Session, days: int = 14) -> dict:
    ...
    return {
        ...
        "activity_by_officer": [{"officer_id": code or "UNASSIGNED", "scans": count, "count": count, "value": count} for code, count in officer_rows],
...
def _evidence_over_time(db: Session, days: int) -> list[dict]:
    ...
    for row in rows:
        ...
        if key in buckets:
            buckets[key] += 1
    return [{"date": day, "count": count, "value": count} for day, count in buckets.items()]
