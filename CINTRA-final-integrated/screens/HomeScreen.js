        ...
        {activeCase ? (
          <View style={styles.activeCaseCard}>
            <View style={styles.caseCardHeader}>
              <BadgeGroup style={styles.caseBadgeRow}>
                <Badge type={activeCase.status} />
                {activeCase.priority && (
                  <Badge type={activeCase.priority} label={`${activeCase.priority} PRIORITY`} />
                )}
              </BadgeGroup>
            ...
            <Text style={styles.caseIdText} numberOfLines={1}>{activeCase.case_id}</Text>
            <Text style={styles.caseTitleText} numberOfLines={2}>{activeCase.title}</Text>

            <View style={styles.caseMetaWrapRow}>
              <View style={styles.metaItem}>
                <Ionicons name="calendar-outline" size={14} color="#666" />
                <Text style={styles.caseMetaText} numberOfLines={1}>Date: {activeCase.date}</Text>
              </View>
              <View style={styles.metaItem}>
                <Ionicons name="person-outline" size={14} color="#666" />
                <Text style={styles.caseMetaText} numberOfLines={1}>Assignment: {activeCase.officer || `Badge ${badgeId}`}</Text>
              </View>
            </View>

            {activeCase.summary ? (
              <Text style={styles.caseSummaryText} numberOfLines={3}>
                {activeCase.summary}
              </Text>
            ) : null}
          </View>
                  ...
                  return (
                    ...
                    >
                      <View style={styles.caseItemHeader}>
                        <Text style={styles.caseItemCode} numberOfLines={1}>{item.case_id}</Text>

                        <BadgeGroup>
                          <Badge type={item.status} />
                          {item.priority ? <Badge type={item.priority} /> : null}
                        </BadgeGroup>
                      </View>

                      <Text style={styles.caseItemTitle} numberOfLines={2}>{item.title}</Text>

                      <View style={styles.caseMetaWrapRow}>
                        <View style={styles.metaItem}>
                          <Ionicons name="person-outline" size={13} color="#666" />
                          <Text style={styles.caseItemMeta} numberOfLines={1}>Assigned: {item.officer || `Badge ${badgeId}`}</Text>
                        </View>
                        <View style={styles.metaItem}>
                          <Ionicons name="calendar-outline" size={13} color="#666" />
                          <Text style={styles.caseItemMeta} numberOfLines={1}>Date: {item.date}</Text>
                        </View>
                      </View>
