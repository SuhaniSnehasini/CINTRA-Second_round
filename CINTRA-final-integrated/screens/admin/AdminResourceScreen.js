  ...
  if (routeName === 'AdminPersons') {
    if (isMobile) {
      return (
        <View style={styles.mobileCardList}>
          {items.map((row, index) => {
            const personId = row.person_id || row.suspect_code || row.suspect_id || row.id || `P-DEMO-${index + 1}`;
            const refImages = row.reference_image_count ?? row.ref_images_count ?? row.num_images ?? (row.reference_images ? row.reference_images.length : 0);
            const recognitions = row.recognition_count ?? row.recognitions_count ?? row.events_count ?? 0;
            const status = row.enrollment_status || row.status || 'REAL_ENROLLMENT';

            return (
              <View key={personId} style={styles.mobileCard}>
                <View style={styles.mobileCardHeader}>
                  <View style={{ flex: 1, minWidth: 0, marginRight: 8 }}>
                    <Text style={styles.mobileKicker}>PERSON ID</Text>
                    <Text style={styles.mobileCodeText} numberOfLines={1}>{personId}</Text>
                  </View>
                  <StatusBadge status={status} />
                </View>
                <View style={styles.mobileCardBody}>
                  <Text style={styles.mobileMetaText}>Ref Images: <Text style={styles.mobileValText}>{refImages} images</Text></Text>
                  <Text style={styles.mobileMetaText}>Recognitions: <Text style={styles.mobileValText}>{recognitions} events</Text></Text>
                  <Text style={styles.mobileMetaText}>Origin: <Text style={styles.mobileValText}>{row.data_origin || 'Operational'}</Text></Text>
                </View>
                <TouchableOpacity
                  style={styles.mobileActionRow}
                  onPress={() => navigation.navigate('AdminPersonDetail', { id: personId })}
                >
    ...
    return (
      <View>
        ...
        {items.map((row, index) => {
          ...
          return (
            <View key={personId} style={styles.trRow}>
              <Text style={[styles.tdCode, { flex: 1.5 }]} numberOfLines={1}>{personId}</Text>
              <View style={{ flex: 1.8 }}>
                <StatusBadge status={status} />
              </View>
              <Text style={[styles.tdText, { flex: 1.2 }]}>{refImages} images</Text>
              <Text style={[styles.tdText, { flex: 1.2 }]}>{recognitions} events</Text>
  ...
  if (routeName === 'AdminEvidence') {
    if (isMobile) {
      return (
        <View style={styles.mobileCardList}>
          {items.map((row) => {
            const integrity = row.integrity_status || row.status || 'VERIFIED';
            const typeName = row.type || row.evidence_type || 'Evidence';
            const officerName = row.officer || row.current_custodian || row.original_badge_id || '—';
            ...
            return (
              <View key={row.evidence_id} style={styles.mobileCard}>
                <View style={styles.mobileCardHeader}>
                  ...
                  <StatusBadge status={integrity} />
                </View>
                <View style={styles.mobileCardBody}>
                  <Text style={styles.mobileMetaText}>Type: <Text style={styles.mobileValText}>{typeName}</Text></Text>
                  <Text style={styles.mobileMetaText}>Officer: <Text style={styles.mobileValText} numberOfLines={1}>{officerName}</Text></Text>
        ...
        {items.map((row) => {
          ...
          return (
            <View key={row.evidence_id} style={styles.trRow}>
              ...
              <Text style={[styles.tdPrimaryText, { flex: 1.2 }]}>{typeName}</Text>
              <Text style={[styles.tdText, { flex: 1.4 }]} numberOfLines={1}>{officerName}</Text>
