...
const handleIdentify = async (
  ...
  try {
    ...
    console.log('[CINTRA Scanner] Identification result:', response);

    // Always use the real response returned by the backend/API.
    // Do not replace a valid "no match" or "multiple faces" response
    // with a hardcoded demo suspect.
    setScanResult(response);
  } catch (error) {
    console.error('[CINTRA Scanner] Identification error:', error);

    // This catch is only for an unexpected error thrown by identifyFace().
    // identifyFace() already contains its own network fallback.
    setScanResult({
      match: false,
      suspect: null,
      message: 'Identification service unavailable. Please try again.',
    });
  } finally {
    setIsIdentifying(false);
  }
