...
export default function CaptureEvidenceScreen({ navigation }) {
  ...
  const handleUploadEvidence = async () => {
    ...
    try {
      ...
      console.log(
        '[CINTRA] Photo evidence upload successful:',
        result
      );
      ...
      Alert.alert(
        'Upload Failed',
        uploadErr.message ||
          'Unable to upload evidence to backend server.'
      );
    } finally {
  ...
  const handleMatchWithDatabase = async () => {
    if (!capturedImage) {
      Alert.alert(
        'No Image',
        'Please capture a photo before matching with database.'
      );
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <Text style={styles.instruction}>
          Capture photographic evidence for investigation,
          verify its SHA-256 hash, and upload directly to
          backend storage.
        </Text>
